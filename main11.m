...
% an attempt is made here to limit alpha_t
...
xf =  680343.127073116;%1e-10;
uf = -452.298273234552;%1e-10;

yf =  6202633.55835012;%1e-10;
vf =  49.617808928807;%1e-10;
...
options = optimoptions('fmincon','Display', 'iter', 'Algorithm', 'interior-point','MaxFunctionEvaluations', 5000000,'MaxIterations',50000,...
                        'FunctionTolerance',1e-2,'OptimalityTolerance',1e-2, ...
...
theta0  = linspace(-deg2rad(90),deg2rad(90),N+1)';
utheta0 = linspace(-deg2rad(0.5),deg2rad(0.5),N+1)';

beta0   = linspace(-deg2rad(90),deg2rad(90),N+1)';
ubeta0  = linspace(-deg2rad(0.5),deg2rad(0.5),N+1)';

alpha_t0 = linspace(deg2rad(170.01),deg2rad(179.99),N+1)';
phi_A0   = linspace(deg2rad(0),deg2rad(170),N+1)';


% P0   = [X0; U0; Y0; V0; Z0;W0; Mg; theta0; beta0; utheta0;ubeta0; tfg]; 
